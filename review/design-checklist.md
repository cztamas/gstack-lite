...
**Trigger:** Only run this checklist if the diff touches frontend files. Use `gl-diff-scope` to detect:

```bash
source <($HOME/.gstack-lite/bin/gl-diff-scope <base> 2>/dev/null)
```
...
- **[LOW]** - Requires understanding visual intent. Present as: "Possible issue - verify visually or run /gl-design-review."
...
**LOW confidence items** -> present as "Possible: [description]. Verify visually or run /gl-design-review." Never AUTO-FIX.
...
- [file:line] Possible issue - verify with /gl-design-review
```
