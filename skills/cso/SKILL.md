...
description: |
  ...
  Use when: "security audit", "threat model", "pentest review", "OWASP", "CSO review". (gstack-lite)
...
# /gl-cso - Chief Security Officer Audit (v2)
...
## User-invocable
When the user types `/gl-cso`, run this skill.

## Arguments
- `/gl-cso` - full daily audit (all phases, 8/10 confidence gate)
- `/gl-cso --comprehensive` - monthly deep scan (all phases, 2/10 bar - surfaces more)
- `/gl-cso --infra` - infrastructure-only (Phases 0-6, 12-14)
- `/gl-cso --code` - code-only (Phases 0-1, 7, 9-11, 12-14)
- `/gl-cso --skills` - skill supply chain only (Phases 0, 8, 12-14)
- `/gl-cso --diff` - branch changes only (combinable with any above)
- `/gl-cso --supply-chain` - dependency audit only (Phases 0, 3, 12-14)
- `/gl-cso --owasp` - OWASP Top 10 only (Phases 0, 9, 12-14)
- `/gl-cso --scope auth` - focused audit on a specific domain
...
The bash blocks throughout this skill show WHAT patterns to search for, not HOW to run them. Use the host's search/read tools (which handles permissions and access correctly) rather than raw bash grep. The bash blocks are illustrative examples - do NOT copy-paste them into a terminal. Do NOT use `| head` to truncate results.
...
```bash
for d in .claude/skills .codex/skills .cursor/skills; do ls -la "$d" 2>/dev/null; done
```
...
**Daily mode (default, `/gl-cso`):** 8/10 confidence gate. Zero noise. Only report what you're sure about.
...
**Comprehensive mode (`/gl-cso --comprehensive`):** 2/10 confidence gate. Filter true noise only (test fixtures, documentation, placeholders) but include anything that MIGHT be a real issue. Flag these as `TENTATIVE` to distinguish from confirmed findings.
...
**Trend Tracking:** If prior reports exist in `.gstack-lite/security-reports/`:
```
...
3. Options:
   ...
   - D) Defer to the project TODO tracker with the repository's security label, field, or equivalent classification. If the user chooses D, record it in the destination resolved by the preamble and report its identifier.

### Phase 14: Save Report

```bash
mkdir -p .gstack-lite/security-reports
```

Write findings to `.gstack-lite/security-reports/{date}-{HHMMSS}.json` using this schema:
...
If `.gstack-lite/` is not in `.gitignore`, note it in findings - security reports should stay local.
...
**This tool is not a substitute for a professional security audit.** /gl-cso is an AI-assisted
...
sensitive data, payments, or PII, engage a professional penetration testing firm. Use /gl-cso as
...
**Always include this disclaimer at the end of every /gl-cso report output.**
