...
description: |
  ...
  Proactively suggest when user has approved a design or has a plan ready. (gstack-lite)
...
3. If in plan-driven or freeform mode (no approved PNG), design from context:
   ...
   - **Freeform:** ask the user to gather what the user wants to build. Ask about:
...
_PRETEXT_VENDOR=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.gstack-lite/design-html/vendor/pretext.js" ] && _PRETEXT_VENDOR="$_ROOT/.gstack-lite/design-html/vendor/pretext.js"
[ -z "$_PRETEXT_VENDOR" ] && [ -f $HOME/.gstack-lite/design-html/vendor/pretext.js ] && _PRETEXT_VENDOR=$HOME/.gstack-lite/design-html/vendor/pretext.js
...
LOOP:
  ...
  3. user question (adjust wording based on mode):
...
Maximum 10 iterations. If the user hasn't said "done" after 10, ask the user:
