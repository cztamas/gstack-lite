...
| Mode | full | `--regression .gstack-lite/qa-reports/baseline.json` |
| Output dir | `.gstack-lite/qa-reports/` | `Output to /tmp/qa` |
...
[ -f .gstack-lite/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
**If NO runtime detected** (no config files found): Ask the user:
"I couldn't detect your project's language. What runtime are you using?"
Options: A) Node.js/TypeScript B) Ruby/Rails C) Python D) Go E) Rust F) PHP G) Elixir H) This project doesn't need tests.
If user picks H -> write `.gstack-lite/no-test-bootstrap` and continue without tests.
...
If user picks C -> write `.gstack-lite/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.gstack-lite/no-test-bootstrap` and re-run." Continue without tests.
...
```bash
mkdir -p .gstack-lite/qa-reports/screenshots
```
...
1. **Project plan:** Resolve the project through the Project Plan Structure protocol. Read `status.md`, then the `## Test Plan` and verification sections of `plan.md`.
2. **Supplementary checklist:** Follow any specifically named QA or test-plan file linked from the project files.
3. **Conversation context:** Check whether a prior planning review produced additional test-plan decisions in this conversation.
4. **Use whichever source is richer.** Fall back to git diff analysis only if none is available.
...
gstack-browser screenshot "$REPORT_DIR/screenshots/issue-NNN-after.png"
gstack-browser console --errors
gstack-browser snapshot -D
```
...
Write the normal local report:

**Local:** `.gstack-lite/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`

When the outcome is substantial and should survive as project context, additionally write a specifically named report such as `<project-dir>/qa-report-{YYYY-MM-DD}.md` and link it from `plan.md`. Do not create a second project-scoped artifact for routine successful QA.
...
Update `status.md` with the QA outcome, remaining blockers, and exact next action. Set `complete` only when the plan's completion criteria are actually satisfied; otherwise preserve the appropriate current status.
...
11. **Clean working tree required.** If dirty, ask the user to offer commit/stash/abort before proceeding.
